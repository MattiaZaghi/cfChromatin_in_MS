# Load the Biobase library which provides container structures for the analysis and comprehension of high-throughput genomic data.
library(Biobase)

# Source additional R scripts containing utility functions required for the analysis.
source(paste0(SourceDIR, "QQNormalize.R")) # Script for quantile-quantile normalization of data.
source(paste0(SourceDIR, "MetaPlot.R")) # Script for creating meta plots.
source(paste0(SourceDIR, "cfChIP-util.R")) # Utility functions for cfChIP analysis.
source(paste0(SourceDIR, "PlotPVal.R")) # Functions for plotting p-values.
source(paste0(SourceDIR, "YieldEstimation-Functions.R")) # Functions for estimating yield.
source(paste0(SourceDIR, "EstimateGammaPoisson.R")) # Functions for estimating parameters of a Gamma-Poisson distribution.
source(paste0(SourceDIR, "NMF-util.R")) # Utility functions for non-negative matrix factorization.

# Set the maximum vector size to infinite to prevent memory size issues during computation.
mem.maxVSize(vsize = Inf)

# Create a data frame containing descriptions for the windows, including type, gene, and tissue information.
# This data frame is used for annotating the windows in the analysis.
WinDescription = data.frame(Type = TSS.windows$type, 
                            Gene = TSS.windows$name, 
                            Tissue = TSS.windows$tissue, 
                            stringsAsFactors = FALSE)
# Set the row names of the WinDescription data frame to be sequential numbers.
rownames(WinDescription) = 1:nrow(WinDescription)

# Identify the names of windows that are not excluded from the analysis.
Win.notexcluded.name = which(Win.notexcluded)

# Define a function to compute the logarithm of the sum of two exponentiated numbers in a numerically stable way.
logSum = function(a, b) {
  m = max(a,b)
  m + log(exp(a-m)+exp(b-m))
}

# Define a function to compute the logarithm of the average of two exponentiated numbers in a numerically stable way.
logAvg = function(a, b) {
  m = max(a,b)
  m + log(exp(a-m)+exp(b-m))-log(2)
}

# Define a function to compute a two-tailed p-value based on two one-tailed p-values.
...
cfChIP.Params <- function() {
  list( 
    ...
    PlotSignatures.MaxCount = 100,
    PlotSignatures.MaxZscore = 300,
...
cfChIP.ProcessFile <- function(filename = NULL,
  ...
  if(is.null(dat$Counts)) {
    ...
    if(!is.null(dat$BED)) {
      ...
      ChrRle = S4Vectors::Rle(chrom(params$TSS.windows))
      ChrStarts = start(ChrRle)
      ChrEnds = end(ChrRle)
      ChrName = as.character(runValue(ChrRle))
      for(i in 1:length(ChrRle)) {
        catn(ChrName[i])
