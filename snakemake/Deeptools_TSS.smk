...
CHIP="cfH3K27ac_H15_nano_TSS".split() # Bigwig tracks to plot
...
rule Compute_Matrix:
    input:
        #bigwig1='/proj/user/mattia/cfChromatin_CombatMS_pA_hg38/coverage/deeptools/H15-P-Ctrl_H3K27ac_ChIP_RPKM.bw',
        bigwig1='/date/gcb/gcb_MZ/S3_nanoCT/coverage/deeptools/H15_H3K27ac_S3-nanoCT_RPKM.bw',
        bigwig2='/date/gcb/gcb_MZ/S3_nanoCT/coverage/deeptools/H15_H3K27ac_nanoCT_RPKM.bw',
    ...
    shell:
        "computeMatrix  scale-regions -S {input.bigwig1} {input.bigwig2}  -R {input.BED} -o {output.Compute_Matrix} -a 3000 -m 1000 -b 3000 -p {threads} --averageTypeBins {wildcards.stat} "
...
rule Plot_heatmap_kmeans: #this is the code used to obtain clusters of differential chromatin accessibility in different samples and cell types
    ...
    shell:
        """
        plotHeatmap --matrixFile {input.Compute_Matrix} --outFileName {output.Heatmap_kmeans} --heatmapWidth 8 --heatmapHeight 20 --dpi 300 --sortRegions no --averageTypeSummaryPlot {wildcards.stat} --kmeans {wildcards.kmeans}  --refPointLabel center --colorMap  Blues Greens   --missingDataColor  lightgrey --whatToShow  'plot, heatmap and colorbar' --samplesLabel   "H15-S3" "H15-nano"  --startLabel "5'" --endLabel "3'" --legendLocation none --outFileSortedRegions  {output.Heatmap_bed}  --zMax {wildcards.zmax} 
        """
